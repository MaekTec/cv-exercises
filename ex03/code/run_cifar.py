...
from lib.cifar_dataset import create_cifar_datasets, create_dataloader, get_dataloaders
...
def get_transforms(args):
    if args.transforms == 'basic':
        ...
        train_transforms = torchvision.transforms.Compose(
            [horizontal_flip(0.5), random_resize_crop(32, (0.3, 1.0)),
             torchvision.transforms.ToTensor(),
             torchvision.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
        )
        # END TODO #################
        ...
        train_transforms = torchvision.transforms.Compose(
            ...
            torchvision.transforms.RandomResizedCrop(32, (0.3, 1.0)),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        )
        # END TODO #################
...
def evaluate_one_epoch(model, test_loader, epoch, device, loss_fn, args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, label) in enumerate(test_loader):
            ...
            # log the loss
            if batch_idx % 100 == 0:
                print(f"Validation of epoch {epoch}/{args.num_epochs} step {batch_idx}/{len(test_loader)}")
...
def main():
    ...
    parser.add_argument('--transforms', type=str, default='basic',help='which transformations to use', choices=['basic', 'own', 'torchvision'])
    ...
    tb_writer = SummaryWriter(log_dir=os.path.join(args.out_dir, "logs"))
    # END TODO #################
    ...
    if args.test_dataloader:
        ...
        for batch_idx, (data, label) in enumerate(train_loader):
            print(f"Batch {batch_idx}, data {data.shape} {data.dtype}, labels {label.shape} {label.dtype}")
            if batch_idx == 3:
                break
    ...
    tb_writer.add_scalar('Accuracy/val', test_acc, epoch)
    # END TODO ###################
...
if __name__ == '__main__':
    ...
    #   or
    #   (2.2) spot the vc-code popup that says the application is now running and press 'open-in-local-browser'
